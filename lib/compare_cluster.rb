...
class CompareCluster
  ...
  def compare
    ...
    compare_htitems
    (compare_holdings_keys && compare_holdings_values)
  end

  private

  def cluster_overlap_debugging
    puts "Calculated format for cluster: #{CalculateFormat.new(@cluster).cluster_format}"

    debug_htitems
    debug_holdings_keys
    debug_holdings_contents
  end
