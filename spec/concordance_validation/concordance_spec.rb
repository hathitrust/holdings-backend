...
RSpec.describe ConcordanceValidation::Concordance do
  ...
  describe '#numbers_tab_numbers' do
    ...
    it 'raises an error if it isn\'t numbers-tab-numbers throughout' do
      expect { described_class.numbers_tab_numbers('spec/concordance_validation/data/cycles.tsv') }.not_to \
        raise_error
    end
  end
  ...
  describe 'described_class.new' do
    it 'builds a basic concordance structure' do
      expect(described_class.new('spec/concordance_validation/data/chained.tsv').raw_to_resolved).to \
        eq(1 => [2], 2 => [3])
    end

    it 'handles gzipped files' do
      expect(described_class.new('spec/concordance_validation/data/chained.tsv.gz').raw_to_resolved).to \
        eq(1 => [2], 2 => [3])
    end
  end
