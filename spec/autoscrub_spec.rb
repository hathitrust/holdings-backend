...
RSpec.describe Autoscrub do
  ...
  describe "member_id handling" do
    ...
    it "rejects a bad member_id" do
      expect { described_class.new("failme") }.to raise_error(MemberIdError)
    end

    it "rejects an empty member_id" do
      expect { described_class.new("") }.to raise_error(MemberIdError)
    end

    it "rejects a nil member_id" do
      expect { described_class.new(nil) }.to raise_error(MemberIdError)
    end
  end
  ...
  describe "input file header" do
    ...
    it "rejects header lines that are not well formed" do
      ...
      expect(autoscrub.well_formed_header?(
        ["oclc", "local_id", "enumchron"], "mono"
)).to be(false)
      expect(autoscrub.well_formed_header?(
        ["oclc", "local_id", "enumchron"], "multi"
)).to be(true)
      expect(autoscrub.well_formed_header?(
        ["oclc", "local_id", "enumchron"], "serial"
)).to be(false)
    end
  end
  ...
  describe "field value extraction" do
    ...
    it "accepts ok issns" do
      ["1234-1234",
       ...
       "1234123X",
       "1234-1234;2345-2345",
       "1234-123x;2345-234X"].each do |issn|
    ...
    it "partially accepts mixed good/bad  issns" do
      ["1234-1234;1234-SAUL",
       ...
       "1234-1234;2345-23456"].each do |issn|
  ...
  describe "enumchron normalization" do
    ...
    it "currently allows anything in enumchron" do
      ...
      expect(autoscrub.check_col_val(ec, "1970,v.1")).to  eq(["1", "1970"])
      expect(autoscrub.check_col_val(ec, "1970, v.1")).to eq(["1", "1970"])
      expect(autoscrub.check_col_val(ec, "V.8 How to Price Meats")).to eq(["8:How:to:Price:Meats", nil])
